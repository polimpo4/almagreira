...
function cs_shortocde_selection(){
	...
	if(v == 'toogle'){
		...
		send_to_editor('[message_box align="left" icon="icon-info-sign" close="yes" color="#COLOR_CODE" background="#COLOR_CODE" border_color="#COLOR_CODE" box_shadow_color="#COLOR_CODE" title="Message Title"]Message Content[/message_box]<br /><br />');
	  }
   else if(v == 'frame'){
		send_to_editor('[frame src="IMAGE_SOURCE" width="IMAGE_WIDTH" height="IMAGE_HEIGHT" lightbox="on" title="Image Title"]<br /><br />');
	  }
   else if(v == 'list'){
		send_to_editor('[list type="decimal" icon="icon-ok"]<br />\
		...
		send_to_editor('[video-item url="" width="400" height="250"][/video-item]<br /><br />');
			...
			send_to_editor('[image-frame style="frame1" width="200" height="150" lightbox="yes" source="" caption="Image Caption"][/image-frame]<br /><br />');
	  }
   else if(v == 'icon'){
			send_to_editor('[icon class="icon-twitter" type="rounded" size="icon-4x" border="no" bgcolor="#000" color="#ff0000"][/icon]<br /><br />');
	  }
   else if(v == 'code'){
