...
function cs_shortcode_pb_divider($atts) {
    global $cs_node;
    if (!isset($atts['backtotop'])) {
        $atts['backtotop'] = '';
    }
    if (!isset($atts['style'])) {
        $atts['style'] = '';
    }
    if (!isset($atts['top_margin'])) {
        $atts['top_margin'] = '';
    }
...
function cs_shortcode_pb_message_box($atts, $content = "") {
    ...
    if (!isset($atts['box_shadow_color'])) {
        $atts['box_shadow_color'] = '';
    }
    if (!isset($atts['title'])) {
        $atts['title'] = '';
    }
...
function cs_shortcode_pb_map($atts, $content = "") {
    ...
    if (!isset($atts['type']))
        $atts['type'] = '';
    if (!isset($atts['info_text']))
        $atts['info_text'] = '';
    if (!isset($atts['info_width']))
        $atts['info_width'] = '';
    if (!isset($atts['info_height']))
        $atts['info_height'] = '';
    if (!isset($atts['marker_icon_url']))
        $atts['marker_icon_url'] = '';
    if (!isset($atts['marker']))
        $atts['marker'] = '';
    if (!isset($atts['draggable']))
        $atts['draggable'] = '';
    if (!isset($atts['disable_controls']))
        $atts['disable_controls'] = '';
    if (!isset($atts['scrollwheel']))
        $atts['scrollwheel'] = '';
    if (!isset($atts['map_view']))
        $atts['map_view'] = 'content';
...
function cs_shortcode_pb_code($atts, $content = "") {
    ...
    if (!isset($atts['title'])) {
        ...
        $title = '<header><h2 class="section-title">' . $atts["title"] . '</h2></header>';
    }
...
function cs_shortcode_pb_testimonials($atts, $content = "") {
    ...
    foreach ($cs_xmlObject as $cs_node) {
        $testimonial_count++;
        if (!isset($cs_node["image"])) {
            $cs_node["image"] = '';
        }
        if (!isset($cs_node["name"])) {
            $cs_node["name"] = '';
        }
        if (!isset($cs_node["job"])) {
            $cs_node["job"] = '';
        }
...
function cs_shortcode_pb_team($atts, $content = "") {
    ...
    foreach ($cs_xmlObject as $cs_node) {
        ...
        $html .= '<div class="text">';
        $html .= '<h3 class="uppercase"><a class="colrhover">' . $cs_node["name"] . '</a></h3>';
        ...
        foreach ($cs_node as $cs_node1) {
            foreach ($cs_node1 as $cs_node2) {
                if (!isset($cs_node2["link"])) {
                    $cs_node2["link"] = '';
                }
                if (!isset($cs_node2["icon"])) {
                    $cs_node2["icon"] = '';
                }
    ...
    if (isset($atts["style"]) && $atts["style"] == 'team-v1') {
        $html = '<div class="team-shortcode team-slide">' . $html . '</div>';
    } else {
