...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            $twitter .= "<div id='tweet-1' class='tweet webkit'><h6 class='heading-color'>" . $tweet . "</h6>";

            if (isset($style)) {
                if (!empty($style)) {
                    //$when = ($style == 'time_since' ? 'About' : 'On');
                    $when = ($style == 'time_since' ? '' : '');
                    $twitter.="<p><a class='hour colrhover'>" . $when . "";
    ...
    echo '<div class="twitter_sign webkit"><div class="tweets-wrapper">' . $twitter . '</div>';
